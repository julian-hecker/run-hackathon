...
async def test_rag_agent():
    ...
    user_input = textwrap.dedent("""Who are you?""").strip()
    ...
    assert "rag" in response.lower()


@pytest.mark.asyncio
async def test_scheduling_agent():
    """Runs the scheduling agent on a simple input and expects a normal response."""
    user_input = textwrap.dedent(
        """Schedule an appointment for tomorrow at 10am."""
    ).strip()

    runner = InMemoryRunner(agent=scheduling_agent)
    session = await runner.session_service.create_session(
        app_name=runner.app_name, user_id="test_user"
    )
    content = UserContent(parts=[Part(text=user_input)])
